from solvers.factory import get_solver, new_solver
solver = get_solver()
...
def get_id(x):
    return solver.get_ast_id(x.ctx.ref(), x.as_ast())
...
class SubsetSolver:
    ...
    def __init__(self, constraints):
        ...
        for i in range(self.n):
            self.s.add(solver.implies(self.c_var(i), constraints[i]))

    def c_var(self, i):
        if i not in self.varcache:
            v = solver.bool(str(self.constraints[abs(i)]))
            self.idcache[get_id(v)] = abs(i)
            if i >= 0:
                self.varcache[i] = v
            else:
                self.varcache[i] = solver._neg(v)
        return self.varcache[i]

    def check_subset(self, seed):
        assumptions = self.to_c_lits(seed)
        return (self.s.check(assumptions) == solver._sat())
...
class MapSolver:
    ...
    def next_seed(self):
        ...
        """
        if self.solver.check() == solver._unsat():
            return None
        seed = self.all_n.copy()  # default to all True for "high bias"
        model = self.solver.model()
        for x in model:
            if solver.is_false(model[x]):
    ...
    def block_down(self, frompoint):
        ...
        self.solver.add(solver._or([solver.bool(str(i)) for i in comp]))

    def block_up(self, frompoint):
        """Block up from a given set."""
        self.solver.add(solver._or([solver._neg(solver.bool(str(i))) for i in frompoint]))
